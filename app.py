...
if int(id_) in ratings.userId.unique():
  for j in top_n[str(int(id_))]:
    try:
      ...
      doc = pq('https://www.themoviedb.org/movie/' + str(links.loc[movies['movieId']==j[0]]['tmdbId'].values[0]))
...
if button:
  ...
  for i in range(len(films)):
    ...
    film_link = 'https://www.themoviedb.org/movie/' + str(links.loc[movies['movieId'] == film_ID]['tmdbId'].values[0])
    st.image(img, caption="""#### **[{}]({})**""".format(film_name, film_link), use_column_width=True)
